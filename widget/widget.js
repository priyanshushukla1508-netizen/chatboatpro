(function () {
    ...
    // Define isDemo early so it can be checked anywhere in the script
    ...
    chatWindow.innerHTML = `
        ...
        <div class="chat-input">
            <div class="voice-btn" id="voice-trigger" title="Speak to AI">🎤</div>
    ...
    // Demo Mode Logic (isDemo defined at top of script)
    ...
    // GLOBAL TRIGGER (exposed so sites can open the bot programmatically)
    ...
    // Increment time tracker every 1s for accurate proactive triggering
    let proactiveFired = false;
    setInterval(() => {
        behavior.timeOnPageSeconds += 1;

        // === PROACTIVE TRIGGER LOGIC ===
        // If user is on page for 10 seconds (for demo purposes) and hasn't opened the chat
        if (!proactiveFired && behavior.timeOnPageSeconds >= 10 && !chatWindow.classList.contains('show')) {
            proactiveFired = true;

            // Add a visual notification dot to the bubble before opening
            const notifDot = document.createElement('div');
            notifDot.style.cssText = "position:absolute; top:-5px; right:-5px; width:12px; height:12px; background:#FF4D4D; border-radius:50%; border:2px solid var(--surface-color);";
            bubble.appendChild(notifDot);
            bubble.style.animation = "pulse 1.5s infinite";

            // Open chat and send proactive message
            setTimeout(() => {
                chatWindow.classList.add('show');
                bubble.style.display = 'none';

                let greeting = "Hello! Mujhe aapki activity se lag raha hai aapko help chahiye. Can I assist you with something?";
                if (behavior.visitedPricing) {
                    greeting = "Bhai, pricing page par 10 seconds se ho! Discount offer bataun kya? 🤫💸";
                }

                // Add the bot message
                const msgDiv = document.createElement('div');
                msgDiv.className = 'message bot-message';
                msgDiv.innerHTML = `<div class="message-content">${greeting}</div>`;
                messagesContainer.appendChild(msgDiv);
                messagesContainer.scrollTop = messagesContainer.scrollHeight;
            }, 3000); // Wait 3 seconds after notification dot appears
        }
    }, 1000);
    ...
    // === LIVE CHAT SYNC (Polling for Admin Replies) ===
    let lastMessageCount = 0;
    setInterval(async () => {
        if (!chatWindow.classList.contains('show') || isDemo) return;
        try {
            ...
            if (messages.length > lastMessageCount) {
                // Re-render chat to include new admin messages (Filters out INTERNAL_NOTE via backend)
                messagesContainer.innerHTML = '';
                messages.forEach(m => {
                    const sender = m.senderType === 'VISITOR' ? 'user' : 'bot';

                    const msgDiv = document.createElement('div');
                    msgDiv.className = `message ${sender}-message`;
                    if (m.senderType === 'ADMIN') {
                        // Style admin messages slightly differently to show it's a human
                        msgDiv.innerHTML = '<div class="message-content" style="background: linear-gradient(135deg, var(--primary-color), var(--secondary-color));"><div style="font-size:10px; opacity:0.8; margin-bottom:4px">👨‍💼 Human Agent</div>' + m.message + '</div>';
                    } else {
                        msgDiv.innerHTML = '<div class="message-content">' + m.message + '</div>';
                    }
                    messagesContainer.appendChild(msgDiv);
                });
                messagesContainer.scrollTop = messagesContainer.scrollHeight;
    ...
    // === VOICE SUPPORT (STT) ===
    const voiceTrigger = shadow.getElementById('voice-trigger');
    ...
    if (SpeechRecognition) {
        ...
        recognition.onend = () => { voiceTrigger.classList.remove('active'); };
        recognition.onerror = () => { voiceTrigger.classList.remove('active'); };
    } else {
